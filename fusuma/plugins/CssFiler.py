...
def fsm_add_handlers(fsm):
    fsm.append_url_handler( re.compile(r"^/filer_css/"), _filer_css )
    fsm.append_url_handler( re.compile(r"^/editor_css/"), _editor_css )

def _editor_css(self):
    """
    Handler for http://server.name/<cgi_pathname>/editor/
    """
    root_fullpath = self.get_config( "css" )
    root_url = self.script_name() + "/editor_css/"
    file = self.path_info().replace( "/editor_css/", "" )
    file_fullpath = os.path.join( root_fullpath, file )

    root_filer = self.script_name() + "/filer_css/"
    ...
    print self.parse_template( "editor_css", args )
...
def _viewer_css(self):
    ...
    """
    root_fullpath = self.get_config( "css" )
    root_url = self.script_name() + "/viewer_css/"
    current_dir = self.path_info().replace( "/viewer_css/", "" )
...
def _filer_css(self):
    ...
    root_fullpath = self.get_config( "css" )
    root_url = self.script_name() + "/filer_css/"
    viewer_url = self.script_name() + "/viewer_css/"
    editor_url = self.script_name() + "/editor_css/"

    current_dir = self.path_info().replace( "/filer_css/", "" )
    current_fullpath = os.path.join( root_fullpath, current_dir )
