...
def fsm_add_handlers(fsm):
    fsm.append_url_handler( re.compile(r"^/filer_flavours/"), _filer_flavours )
    fsm.append_url_handler( re.compile(r"^/editor_flavours/"), _editor_flavours )

def _editor_flavours(self):
    """
    Handler for http://server.name/<cgi_pathname>/editor/
    """
    root_fullpath = self.get_config( "flavours" )
    root_url = self.script_name() + "/editor_flavours/"
    file = self.path_info().replace( "/editor_flavours/", "" )
    file_fullpath = os.path.join( root_fullpath, file )

    root_filer = self.script_name() + "/filer_flavours/"
    ...
    print self.parse_template( "editor_flavours", args )
            
def _viewer_flavours(self):
    """
    Handler for http://server.name/<cgi_pathname>/viewer/
    """
    root_fullpath = self.get_config( "flavours" )
    root_url = self.script_name() + "/viewer_flavours/"
    current_dir = self.path_info().replace( "/viewer_flavours/", "" )
    current_fullpath = os.path.join( root_fullpath, current_dir )

def _filer_flavours(self):
    """
    Handler for http://server.name/<cgi_pathname>/filer/
    """

    root_fullpath = self.get_config( "flavours" )
    root_url = self.script_name() + "/filer_flavours/"
    viewer_url = self.script_name() + "/viewer_flavours/"
    editor_url = self.script_name() + "/editor_flavours/"

    current_dir = self.path_info().replace( "/filer_flavours/", "" )
    current_fullpath = os.path.join( root_fullpath, current_dir )

    if ( not os.access( current_fullpath, os.R_OK ) ):
        print self.http_header()
        print "<html><body>error: no accessable permittion."
        print current_fullpath, root_fullpath, current_dir
        print "</body></html>"
        return
